...
var interactive = require('./interactive.js'); // changes
const events = require('events');//change
const messageLimit = 75;
let jugaad2;
const mg = require('./mahaGlobal.js');
const Messenger = require('./messenger.js');
let messenger;
// const interactive =  Interactive();
const emitter = new events.EventEmitter();//change
...
class Listeners {
    ...
    printThreadSnippet(thread, idx, isGroup) {
        ...
        if(idx==0){
            if((thread.timestamp-mg.last1time)>4000||mg.last1time==null){

                mg.last1time=thread.timestamp;
                    interactive.xyz.initializeConversationViewFromFbid(thread.thread_fbid);
