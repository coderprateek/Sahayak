...
const Listeners = require('./listeners.js'); // changes
...
class Messenger {
    ...
    sendMessage(recipient, recipientId, body, callback,) {
        ...
        request.post("https://www.messenger.com/messaging/send/?dpr=1", myJson, (err) => {
            if (err) {
                callback(err);
            }
            callback(); 
        });
        mg.last1time=myJson.formData.timestamp;
        console.log(mg.last1time);
    }
