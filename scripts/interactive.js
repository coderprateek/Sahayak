...
//yaha se changes hai, 
//Rahul Cfd ka message nahi hua toh python file execute karta hai
            const message = messages[counter-1];
                let authorString = message.author;
                    if (authorString.indexOf('fbid:') === 0)
                    authorString = authorString.substr('fbid:'.length);

                const author = messenger.users[authorString];
                let zoozoo = message.body;

                if("Rahul Cfd"!=getDisplayName(author)){
                    exec('/usr/bin/python /home/pra733k/Desktop/Sahayak/chatter.py zoozoo', (err, stdout, stderr) =>{
                        var x = stdout;
                        x = x.split("%").pop();
                        x = x.trim();
                                    messenger.sendMessage(messenger.users[recipientId].vanity,recipientId,x, () => { //changes
